-- ============================================================
-- Catalog PoC — feature tests for LAKEKEEPER  (catalog=lakekeeper, schema=ctv_catalog_poc)
-- Generated from scripts/catalog_feature_tests.sql (the <CATALOG>/<SCHEMA> template).
-- PREREQ: the `lakekeeper` Trino catalog must be wired first (runbook Part B) — until then these
-- statements will fail with "catalog 'lakekeeper' does not exist".
...
DROP TABLE IF EXISTS lakekeeper.ctv_catalog_poc.zz_feat_test;

-- 1. v3 CREATE with a VARIANT column  (R1 — the make-or-break)
CREATE TABLE lakekeeper.ctv_catalog_poc.zz_feat_test (
    id      bigint,
    source  varchar,
    payload variant
...
SHOW CREATE TABLE lakekeeper.ctv_catalog_poc.zz_feat_test;   -- expect format_version = 3, payload variant
DESCRIBE lakekeeper.ctv_catalog_poc.zz_feat_test;

-- 3. INSERT VARIANT payloads  (R1)
INSERT INTO lakekeeper.ctv_catalog_poc.zz_feat_test VALUES
...
FROM lakekeeper.ctv_catalog_poc.zz_feat_test ORDER BY id;

-- 5. Row-level DML on v3 (UPDATE / DELETE / MERGE)
UPDATE lakekeeper.ctv_catalog_poc.zz_feat_test
   SET payload = CAST(JSON '{"advertiser":"ACME-2","spend":200}' AS VARIANT) WHERE id = 1;
DELETE FROM lakekeeper.ctv_catalog_poc.zz_feat_test WHERE id = 2;
MERGE INTO lakekeeper.ctv_catalog_poc.zz_feat_test t
...
FROM lakekeeper.ctv_catalog_poc.zz_feat_test ORDER BY id;   -- expect id 1 (ACME-2), 3 (null), 4 (Initech)

-- 6. VIEWS  (soft requirement)
CREATE OR REPLACE VIEW lakekeeper.ctv_catalog_poc.zz_feat_view AS
SELECT id, CAST(payload['advertiser'] AS varchar) AS advertiser
FROM lakekeeper.ctv_catalog_poc.zz_feat_test;
SELECT * FROM lakekeeper.ctv_catalog_poc.zz_feat_view ORDER BY id;

-- 7. cleanup
DROP VIEW  IF EXISTS lakekeeper.ctv_catalog_poc.zz_feat_view;
DROP TABLE IF EXISTS lakekeeper.ctv_catalog_poc.zz_feat_test;

-- ============================================================
-- Interpretation:
--  1-4 pass  -> Lakekeeper serves v3 + VARIANT over REST (the make-or-break R1; Nessie failed here).
