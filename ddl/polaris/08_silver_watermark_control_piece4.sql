-- =====================================================================================
-- Piece 4 (creative sync-back) — seed the TIMESTAMP watermarks in polaris.silver.watermark_control.
...
-- watermark_control is partitioned by watermark_name (ddl/polaris/03) -> each process's row is isolated.
-- =====================================================================================
...
WHERE v.watermark_name NOT IN (SELECT watermark_name FROM polaris.silver.watermark_control);
...
--       end_timestamp   = cast((SELECT max(change_dt) FROM polaris.productcentral.productmap) as timestamp(6) with time zone),
