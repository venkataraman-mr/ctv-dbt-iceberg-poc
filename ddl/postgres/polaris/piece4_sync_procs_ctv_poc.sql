...
-- Bodies are VERBATIM from prod, retargeted to the tempwork *_ctv_poc_pol clones (created by
-- ddl/postgres/polaris/piece3_tempwork_ctv_poc_pol.sql and piece4_seed_tempwork_ctv_poc_pol.sql).
-- jobwork.* scratch -> tempwork *_ctv_poc_pol; reference.*/
-- config.*/"template".* left as prod READS. Real creatives.*/ml_results.* are NOT read here.
--
-- Outputs the dbt/Trino sync reads:
--   tempwork.creative_forsync_tmp_ctv_poc_pol         (from sp_dbx_creative_get_changes_for_databricks_ctv_poc_pol)
--   tempwork.component_coding_forsync_tmp_ctv_poc_pol (from sp_dbx_component_get_changes_for_databricks_ctv_poc_pol)
-- Runtime scratch created by the procs: crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol.
-- Inputs the dbt flow writes before CALL: creatives_advert_hold_tmp_ctv_poc_pol / component_hold_creative_tmp_ctv_poc_pol.
--
...
-- DROP PROCEDURE tempwork.sp_dbx_creative_get_changes_for_databricks_ctv_poc_pol(text);

CREATE OR REPLACE PROCEDURE tempwork.sp_dbx_creative_get_changes_for_databricks_ctv_poc_pol(IN timestamp_string text)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
--
-- Description:
-- This Stored Procedure is used to truncate and create tempwork.creative_forsync_tmp_ctv_poc_pol table
...
-- Also joins the records pushed by databricks which had missing product's reverse_translation i.e tempwork.creatives_advert_hold_tmp_ctv_poc_pol
--
-- Parameters:
...
--
-- Return:
-- Create a table tempwork.creative_forsync_tmp_ctv_poc_pol
--
-- Dependencies:
-- Dependent tables : tempwork.creative_forsync_tmp_ctv_poc_pol, tempwork.creatives_advert_hold_tmp_ctv_poc_pol
--
-- Example Usage:
-- CALL tempwork.sp_dbx_creative_get_changes_for_databricks_ctv_poc_pol(:timestamp_string);
--
-- Revision History:
...
WHERE table_schema = 'tempwork' and
    table_name = 'creative_forsync_tmp_ctv_poc_pol';
if FOUND THEN
    EXECUTE format('truncate tempwork.creative_forsync_tmp_ctv_poc_pol');
ELSE
    CREATE TABLE if not exists tempwork.creative_forsync_tmp_ctv_poc_pol (
...
END if;
	...
	CREATE TABLE IF NOT EXISTS tempwork.creative_classification_engine_holding_ctv_poc_pol (creative_id INT8 NOT NULL, inserted_at TIMESTAMP NOT NULL);

	--Create index on the holding table
	CREATE INDEX IF NOT EXISTS creative_classification_engine_holding_creative_idx_ctv_poc_pol ON ONLY tempwork.creative_classification_engine_holding_ctv_poc_pol USING btree (creative_id);


	--Drop temp table of creatives with holding flag
	DROP TABLE IF EXISTS tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol;


--Get all creatives with updated_timestamp and determine hold flag
CREATE TABLE tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol AS (
    WITH crtvs AS (
        --Get newly updated creatives
        SELECT *
        FROM tempwork.creative_ctv_poc_pol c
        ...
        WHERE EXISTS (
            SELECT 1
            FROM tempwork.creative_classification_engine_holding_ctv_poc_pol holding
    ...
    LEFT JOIN tempwork.creative_first_seen_ctv_poc_pol cfs
    ...
    LEFT JOIN tempwork.creative_ai_classification_staging_vx0_ctv_poc_pol cacs
        ON cacs.creative_id = c.creative_id

	LEFT JOIN tempwork.creative_dedupe_map_ctv_poc_pol cdm ON cdm.child_creative_id = c.creative_id
);
...
CREATE INDEX crtvs_with_hold_flag_creative_idx_ctv_poc_pol ON ONLY tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol USING btree (creative_id);
CREATE INDEX crtvs_with_hold_flag_hold_flag_idx_ctv_poc_pol ON ONLY tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol USING btree (hold_flag);
CREATE INDEX crtvs_with_hold_flag_mime_type_idx_ctv_poc_pol ON ONLY tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol USING btree (creative_mime_type_id);
CREATE INDEX crtvs_with_hold_flag_slogan_tagline_idx_ctv_poc_pol ON ONLY tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol USING btree (attribution_slogan_tagline_id);
CREATE INDEX crtvs_with_hold_flag_url_hash_idx_ctv_poc_pol ON ONLY tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol USING btree (creative_url_hash);
CREATE INDEX crtvs_with_hold_flag_crtv_type_id_idx_ctv_poc_pol ON ONLY tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol USING btree (creative_type_id);
CREATE INDEX crtvs_with_hold_flag_att_status_idx_ctv_poc_pol ON ONLY tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol USING btree (attribution_status_id);
CREATE INDEX crtvs_with_hold_flag_comp_entry_idx_ctv_poc_pol ON ONLY tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol USING btree (component_entry_status_id);
CREATE INDEX crtvs_with_hold_flag_process_step_idx_ctv_poc_pol ON ONLY tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol USING btree (classification_process_step_id);
CREATE INDEX crtvs_with_hold_flag_media_id_idx_ctv_poc_pol ON ONLY tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol USING btree (media_id);
CREATE INDEX crtvs_with_hold_flag_class_type_idx_ctv_poc_pol ON ONLY tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol USING btree (classification_type_id);

--If parent has hold_flag = TRUE then child must get updated to be the same
MERGE INTO tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol TARGET
USING (
  --Get Parents with hold flag = TRUE status and child ids
  SELECT parent_creative_id, child_creative_id
  FROM tempwork.creative_dedupe_map_ctv_poc_pol cdm
  WHERE EXISTS (
    SELECT 1
    FROM tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol hold
...
INSERT INTO tempwork.creative_forsync_tmp_ctv_poc_pol (
 with cte_new_creative as --cte to get all the new updated creatives
  (
    ...
    FROM tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol c
  ...
  (
    ...
    False as is_archive
    FROM
      tempwork.creative_ctv_poc_pol c2
      INNER JOIN tempwork.creatives_advert_hold_tmp_ctv_poc_pol s on s.creative_id = c2.creative_id
    ...
    FROM
      creatives.creative_archive ca
      INNER JOIN tempwork.creatives_advert_hold_tmp_ctv_poc_pol s on s.creative_id = ca.creative_id
  ...
  (
    ...
    FROM
      cte_all_creatives c
      LEFT JOIN tempwork.creative_celebrity_ctv_poc_pol cc on cc.creative_id = c.creative_id
  ...
  (
    ...
    FROM
	  ...
	  cte_all_creatives c
      LEFT JOIN tempwork.creative_occurrence_summary_ctv_poc_pol cs on c.creative_id = cs.creative_id
  ...
  FROM
    cte_creative_attr c
    LEFT JOIN tempwork.creative_product_ctv_poc_pol cpm on c.creative_id = cpm.creative_id
    ...
    LEFT JOIN tempwork.creative_first_seen_ctv_poc_pol cfs on cfs.creative_id = c.creative_id
   	 AND cfs.creative_url_hash = c.creative_url_hash
	 --AND cfs.provider_id = c.provider_id --MRVXVC-15333
    LEFT JOIN tempwork.creative_competitor_ctv_poc_pol cm on cm.creative_id = c.creative_id
...
-- DROP PROCEDURE tempwork.sp_dbx_component_get_changes_for_databricks_ctv_poc_pol(timestamp);

CREATE OR REPLACE PROCEDURE tempwork.sp_dbx_component_get_changes_for_databricks_ctv_poc_pol(IN p_flag_timestamp_cc timestamp without time zone)
 LANGUAGE plpgsql
AS $procedure$
begin
-- =============================================================================
-- Stored Procedure  Name: tempwork.sp_dbx_component_get_changes_for_databricks_ctv_poc_pol
-- Author: Akash Sharma
-- Date Created: 2024-11-11
-- Last Modified By: Akash Sharma on 2025-03-05
--
-- Description:
-- This Stored Procedure is used to truncate and create tempwork.component_coding_forsync_tmp_ctv_poc_pol table
...
-- Also joins the records pushed by databricks which had missing component's product's reverse_translation i.e tempwork.component_hold_creative_tmp_ctv_poc_pol
--
-- Parameters:
-- p_flag_timestamp_cc =  updated timestamp to read the changes from component table
--
-- Return:
-- Create a table tempwork.component_coding_forsync_tmp_ctv_poc_pol
--
-- Dependencies:
-- Dependent tables : tempwork.component_coding_forsync_tmp_ctv_poc_pol, tempwork.component_hold_creative_tmp_ctv_poc_pol
--
-- Example Usage:
...
FROM information_schema.tables
WHERE table_schema = 'tempwork' and
	table_name = 'component_coding_forsync_tmp_ctv_poc_pol';
IF FOUND THEN
  EXECUTE format('truncate tempwork.component_coding_forsync_tmp_ctv_poc_pol');
ELSE
  CREATE TABLE if not exists tempwork.component_coding_forsync_tmp_ctv_poc_pol (
  ...
  order_number int4 null
	...
	INSERT INTO tempwork.component_coding_forsync_tmp_ctv_poc_pol
(
...
WITH cte_comp_crtv as (
	 SELECT
      ...
      order_number
    FROM
      tempwork.component_coding_ctv_poc_pol c
    WHERE
...
--- Vx0 to Vx2 Product translation i.e present in Attribute_response.
    SELECT
      ...
      order_number
    FROM
      tempwork.component_coding_ctv_poc_pol c2
      INNER JOIN tempwork.component_hold_creative_tmp_ctv_poc_pol t ON c2.component_coding_id = t.component_coding_id
  )
SELECT
...
LEFT JOIN tempwork.creative_ctv_poc_pol c
