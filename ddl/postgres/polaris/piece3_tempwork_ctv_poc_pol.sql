...
--
-- Everything lives in the tempwork schema with a _ctv_poc_pol suffix so the PoC never touches
...
--
-- Objects created:
--   tempwork.creative_id_seq_ctv_poc_pol                                (sequence, start 26,000,000,000)
--   tempwork.creative_id_block_ctv_poc_pol + sp_reserve_creative_ids_ctv_poc_pol (Job A id-block reservation)
--   tempwork.creative_staging_ctv_poc_pol                               (clone of creatives.creative_staging)
--   tempwork.creative_first_seen_ctv_poc_pol                            (clone of creatives.creative_first_seen)
--   tempwork.creative_occurrence_summary_ctv_poc_pol                    (clone of creatives.creative_occurrence_summary; Job B)
--   tempwork.sp_dbx_digital_insert_crtv_staging_first_seen_ctv_poc_pol  (Job A insert proc, retargeted)
--   tempwork.sp_dbx_digital_update_raw_occ_to_crtv_first_seen_ctv_poc_pol (Job B first-seen update proc, retargeted)
--   tempwork.sp_dbx_digital_upsert_to_crtv_occ_summary_ctv_poc_pol      (Job B occurrence-summary upsert proc, retargeted)
--
...
--   tempwork.tmp_digital_raw_occ_to_crtv_staging_ctv_poc_pol
--   tempwork.tmp_digital_raw_occ_to_crtv_firstseen_ctv_poc_pol
-- =====================================================================================
...
-- ---------------------------------------------------------------------------------------
CREATE SEQUENCE IF NOT EXISTS tempwork.creative_id_seq_ctv_poc_pol
    AS bigint
...
-- [min, max] block). sp_reserve_creative_ids_ctv_poc_pol(n) atomically pops n values off the sequence
...
-- ---------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tempwork.creative_id_block_ctv_poc_pol (
...
CREATE OR REPLACE PROCEDURE tempwork.sp_reserve_creative_ids_ctv_poc_pol(IN p_n integer)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
    v_start bigint;
...
BEGIN
    ...
    FROM (SELECT nextval('tempwork.creative_id_seq_ctv_poc_pol') AS v
          FROM generate_series(1, p_n)) s;
    INSERT INTO tempwork.creative_id_block_ctv_poc_pol (n, block_start, block_end)
...
-- ---------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tempwork.creative_staging_ctv_poc_pol (
    ...
    CONSTRAINT creative_staging_ctv_poc_pol_pkey PRIMARY KEY (creative_id),
    CONSTRAINT creative_staging_ctv_poc_pol_creative_url_hash_key UNIQUE (creative_url_hash)
);

-- ---------------------------------------------------------------------------------------
-- Clone: creative_first_seen  (columns verbatim; PK + unique(creative_url_hash) only, no triggers).
-- ---------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tempwork.creative_first_seen_ctv_poc_pol (
    creative_id                     int8      NOT NULL,
    ...
    CONSTRAINT creative_first_seen_ctv_poc_pol_pkey PRIMARY KEY (creative_id),
    CONSTRAINT creative_first_seen_ctv_poc_pol_creative_url_hash_key UNIQUE (creative_url_hash)
);
...
-- tempwork _ctv_poc_pol clones. reference.data_provider join kept (read-only prod lookup).
-- Example: CALL tempwork.sp_dbx_digital_insert_crtv_staging_first_seen_ctv_poc_pol('tempwork.tmp_digital_raw_occ_to_crtv_staging_ctv_poc_pol');
-- ---------------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE tempwork.sp_dbx_digital_insert_crtv_staging_first_seen_ctv_poc_pol(IN p_tmp_tbl_name character varying)
 LANGUAGE plpgsql
AS $procedure$
BEGIN

        EXECUTE format('INSERT INTO tempwork.creative_staging_ctv_poc_pol(
        creative_id,
        legacy_creative_id,
        country_iso_2_code,
        provider_code,
...
EXECUTE format('INSERT INTO tempwork.creative_first_seen_ctv_poc_pol(
	        creative_id,
			creative_url_hash,
			provider_creative_id,
			country_iso_2_code,
...
-- Example: CALL tempwork.sp_dbx_digital_update_raw_occ_to_crtv_first_seen_ctv_poc_pol('tempwork.tmp_digital_raw_occ_to_crtv_firstseen_ctv_poc_pol');
-- ---------------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE tempwork.sp_dbx_digital_update_raw_occ_to_crtv_first_seen_ctv_poc_pol(IN p_tmp_tbl_name character varying)
 LANGUAGE plpgsql
AS $procedure$
	BEGIN

		EXECUTE FORMAT (
    'UPDATE tempwork.creative_first_seen_ctv_poc_pol AS trg
	...
	RAISE NOTICE 'Records updated in tempwork.creative_first_seen_ctv_poc_pol matching names';
END;
$procedure$
;
...
-- upsert proc, retargeted to tempwork *_ctv_poc_pol. Real creatives.* untouched. Clone starts empty.
...
CREATE TABLE IF NOT EXISTS tempwork.creative_occurrence_summary_ctv_poc_pol (
    ...
    CONSTRAINT creative_occurrence_summary_ctv_poc_pol_pkey PRIMARY KEY (summary_row_id),
    CONSTRAINT creative_occurrence_summary_ctv_poc_pol_ukey
...
-- Example: CALL tempwork.sp_dbx_digital_upsert_to_crtv_occ_summary_ctv_poc_pol('tempwork.tmp_raw_occ_for_crtv_occ_summary_ctv_poc_pol');
CREATE OR REPLACE PROCEDURE tempwork.sp_dbx_digital_upsert_to_crtv_occ_summary_ctv_poc_pol(IN p_tmp_tbl_name character varying)
 LANGUAGE plpgsql
AS $procedure$
BEGIN
        DROP TABLE IF EXISTS tempwork.tmp_raw_crtv_occ_summary_local_psql_ctv_poc_pol;

        EXECUTE FORMAT ('CREATE TABLE tempwork.tmp_raw_crtv_occ_summary_local_psql_ctv_poc_pol AS
        SELECT
                creative_id,
                creative_url_hash,
                country_iso_2_code,
        ...
        MERGE INTO tempwork.creative_occurrence_summary_ctv_poc_pol AS T
        USING tempwork.tmp_raw_crtv_occ_summary_local_psql_ctv_poc_pol as S
        ...
        DROP TABLE IF EXISTS tempwork.tmp_seven_days_raw_crtv_occ_summary_local_psql_ctv_poc_pol;

        EXECUTE FORMAT ('CREATE TABLE tempwork.tmp_seven_days_raw_crtv_occ_summary_local_psql_ctv_poc_pol AS
        SELECT
                source.creative_id,
                ...
                sum(occ_cnt) as occurrence_count
        FROM %s source
        INNER JOIN tempwork.creative_occurrence_summary_ctv_poc_pol occ_summary
        ...
        MERGE INTO tempwork.creative_occurrence_summary_ctv_poc_pol AS T
        USING tempwork.tmp_seven_days_raw_crtv_occ_summary_local_psql_ctv_poc_pol as S
        ON  T.creative_id = S.creative_id
                AND T.creative_url_hash = S.creative_url_hash
                AND T.country_iso_2_code = S.country_iso_2_code
                AND T.media_id = S.media_id
