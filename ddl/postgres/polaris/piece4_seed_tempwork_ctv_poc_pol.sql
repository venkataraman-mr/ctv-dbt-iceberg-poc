-- =====================================================================================
-- Piece 4 PREREQUISITE — seed production creative data into tempwork *_ctv_poc_pol clones.
...
--   tempwork.creative_ctv_poc_pol                                (clone of creatives.creative)
--   tempwork.creative_product_ctv_poc_pol                        (clone of creatives.creative_product)
--   tempwork.creative_celebrity_ctv_poc_pol                      (clone of creatives.creative_celebrity)
--   tempwork.creative_competitor_ctv_poc_pol                     (clone of creatives.creative_competitor)
--   tempwork.creative_dedupe_map_ctv_poc_pol                     (clone of creatives.creative_dedupe_map)
--   tempwork.creative_classification_engine_holding_ctv_poc_pol  (clone of creatives.creative_classification_engine_holding)
--   tempwork.creative_ai_classification_staging_vx0_ctv_poc_pol  (clone of ml_results.creative_ai_classification_staging_vx0)
--   tempwork.component_coding_ctv_poc_pol                        (clone of creatives.component_coding; Piece 4 task 4)
--   tempwork.watermark_control_ctv_poc_pol                       (clone of config.watermark_control; 2 seed rows)
--   tempwork.sp_seed_load_footprint_ctv_poc_pol()                (internal: (re)load one anchor set)
--   tempwork.sp_seed_creative_clones_ctv_poc_pol(p_mode)         (entry point: Mode 1 + Mode 2)
--
-- ALREADY EXISTS (Piece 3; NOT recreated here):
--   tempwork.creative_staging_ctv_poc_pol            (Piece 3 Job A; NOT seeded — Mode 1 driver + reserved-id
...
--   tempwork.creative_first_seen_ctv_poc_pol         (Job B owns rows for OUR creatives; seed ADDS rows only for
--                                                 EXTERNAL parents (prod id, url_hash not in clone staging).)
--   tempwork.creative_occurrence_summary_ctv_poc_pol (same: seed ADDS EXTERNAL-parent rows only; never touches
...
--   creative_url_hash against creative_staging_ctv_poc_pol (the reserved-id authority):
...
-- regenerated) — except creative_occurrence_summary_ctv_poc_pol.summary_row_id (a Job-B bigserial) which
...
-- ---------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tempwork.creative_ctv_poc_pol (
    ...
    CONSTRAINT creative_ctv_poc_pol_pkey PRIMARY KEY (creative_id)
);

-- ---------------------------------------------------------------------------------------
-- creatives.creative_product  (serial4 -> plain int4, value copied; enums kept)
-- ---------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tempwork.creative_product_ctv_poc_pol (
    ...
    CONSTRAINT creative_product_ctv_poc_pol_pkey PRIMARY KEY (creative_product_id),
    CONSTRAINT creative_product_ctv_poc_pol_creative_id_product_id_key UNIQUE (creative_id, product_id)
);

-- ---------------------------------------------------------------------------------------
-- creatives.creative_celebrity  (serial4 -> plain int4, value copied)
-- ---------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tempwork.creative_celebrity_ctv_poc_pol (
    ...
    updated_timestamp     timestamp NOT NULL,
    CONSTRAINT creative_celebrity_ctv_poc_pol_pkey PRIMARY KEY (creative_celebrity_id)
);

-- ---------------------------------------------------------------------------------------
-- creatives.creative_competitor  (serial4 -> plain int4, value copied)
-- ---------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tempwork.creative_competitor_ctv_poc_pol (
    ...
    updated_by_user_id     int4      NOT NULL,
    updated_timestamp      timestamp NOT NULL,
    CONSTRAINT creative_competitor_ctv_poc_pol_pkey PRIMARY KEY (creative_competitor_id)
);
...
-- ---------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tempwork.creative_dedupe_map_ctv_poc_pol (
    ...
    updated_by_user_id            int4        NOT NULL,
    updated_timestamp             timestamp   NOT NULL,
    CONSTRAINT creative_dedupe_map_ctv_poc_pol_pkey PRIMARY KEY (creative_dedupe_map_id),
    CONSTRAINT crtv_child_crtv_url_hash_ctv_poc_pol_ukey UNIQUE (child_creative_id)
);
...
-- ---------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tempwork.creative_classification_engine_holding_ctv_poc_pol (
    creative_id int8      NOT NULL,
    inserted_at timestamp NOT NULL,
    CONSTRAINT creative_classification_engine_holding_ctv_poc_pol_pkey PRIMARY KEY (creative_id)
);

-- ---------------------------------------------------------------------------------------
-- ml_results.creative_ai_classification_staging_vx0  (PK creative_id kept)
-- ---------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tempwork.creative_ai_classification_staging_vx0_ctv_poc_pol (
    creative_id                        int8        NOT NULL,
    ...
    CONSTRAINT classification_staging_ctv_poc_pol_pkey PRIMARY KEY (creative_id)
);
...
-- ---------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tempwork.component_coding_ctv_poc_pol (
    ...
    CONSTRAINT component_coding_ctv_poc_pol_pkey PRIMARY KEY (component_coding_id)
);

-- ---------------------------------------------------------------------------------------
-- config.watermark_control clone + two seed rows (high-water stored in table_tx_end).
--   CTV_POC_SEED_NEW    -> Mode 1 mark on tempwork.creative_staging_ctv_poc_pol.updated_timestamp
--   CTV_POC_SEED_UPDATE -> Mode 2 mark on creatives.creative.updated_timestamp
-- ---------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tempwork.watermark_control_ctv_poc_pol (
    ...
    CONSTRAINT watermark_control_ctv_poc_pol_pkey PRIMARY KEY (watermark_name)
);

INSERT INTO tempwork.watermark_control_ctv_poc_pol
...
--    resolved against creative_staging_ctv_poc_pol (reserved id if the url_hash is ours, else prod id).
...
-- =====================================================================================
CREATE OR REPLACE PROCEDURE tempwork.sp_seed_load_footprint_ctv_poc_pol()
 LANGUAGE plpgsql
AS $procedure$
BEGIN
    -- -----------------------------------------------------------------------------------
    ...
    LEFT JOIN tempwork.creative_staging_ctv_poc_pol s ON s.creative_url_hash = dm.parent_creative_url_hash
    ...
    DELETE FROM tempwork.creative_first_seen_ctv_poc_pol
      WHERE creative_id IN (SELECT clone_creative_id FROM _seed_idmap WHERE is_ours = false);
    INSERT INTO tempwork.creative_first_seen_ctv_poc_pol (
    ...
    DELETE FROM tempwork.creative_dedupe_map_ctv_poc_pol
      WHERE child_creative_id IN (SELECT clone_creative_id FROM _seed_idmap);
    INSERT INTO tempwork.creative_dedupe_map_ctv_poc_pol (
    ...
    JOIN tempwork.creative_staging_ctv_poc_pol    css_c ON css_c.creative_id = mc.clone_creative_id
    JOIN tempwork.creative_first_seen_ctv_poc_pol  cfs_c ON cfs_c.creative_id = mc.clone_creative_id
    JOIN reference.media                        med_c ON med_c.media_id   = cfs_c.media_id
    -- parent descriptors from clone when the parent is one of ours ...
    LEFT JOIN tempwork.creative_staging_ctv_poc_pol    css_p ON css_p.creative_id = mp.clone_creative_id
    LEFT JOIN tempwork.creative_first_seen_ctv_poc_pol  cfs_p ON cfs_p.creative_id = mp.clone_creative_id
    ...
    DELETE FROM tempwork.creative_occurrence_summary_ctv_poc_pol
      WHERE creative_id IN (SELECT clone_creative_id FROM _seed_idmap WHERE is_ours = false);
    INSERT INTO tempwork.creative_occurrence_summary_ctv_poc_pol (
    ...
    WHERE m.clone_creative_id IN (SELECT d.parent_creative_id FROM tempwork.creative_dedupe_map_ctv_poc_pol d JOIN _seed_idmap ci ON ci.clone_creative_id = d.child_creative_id);

    -- 2.5  staging_vx0  [all in-scope creatives; remap creative_id]  (MERGE / upsert on creative_id)
    INSERT INTO tempwork.creative_ai_classification_staging_vx0_ctv_poc_pol (
    ...
    LEFT JOIN tempwork.creative_staging_ctv_poc_pol css ON css.creative_id = m.clone_creative_id  -- our creatives only
    ...
    WHERE m.is_ours
       OR m.clone_creative_id IN (SELECT d.parent_creative_id FROM tempwork.creative_dedupe_map_ctv_poc_pol d JOIN _seed_idmap ci ON ci.clone_creative_id = d.child_creative_id)
    ...
    INSERT INTO tempwork.creative_ctv_poc_pol (
    ...
    LEFT JOIN tempwork.creative_staging_ctv_poc_pol    css ON css.creative_id = m.clone_creative_id  -- our creatives only
    LEFT JOIN tempwork.creative_first_seen_ctv_poc_pol  cfs ON cfs.creative_id = m.clone_creative_id  -- our creatives only
    ...
    DELETE FROM tempwork.creative_product_ctv_poc_pol
      WHERE creative_id IN (SELECT clone_creative_id FROM _seed_idmap);
    INSERT INTO tempwork.creative_product_ctv_poc_pol
    ...
    DELETE FROM tempwork.creative_celebrity_ctv_poc_pol
      WHERE creative_id IN (SELECT clone_creative_id FROM _seed_idmap);
    INSERT INTO tempwork.creative_celebrity_ctv_poc_pol
    ...
    DELETE FROM tempwork.creative_competitor_ctv_poc_pol
      WHERE creative_id IN (SELECT clone_creative_id FROM _seed_idmap);
    INSERT INTO tempwork.creative_competitor_ctv_poc_pol
    ...
    DELETE FROM tempwork.component_coding_ctv_poc_pol
      WHERE creative_id IN (SELECT clone_creative_id FROM _seed_idmap);
    INSERT INTO tempwork.component_coding_ctv_poc_pol
    ...
    INSERT INTO tempwork.creative_classification_engine_holding_ctv_poc_pol (creative_id, inserted_at)
...
-- =====================================================================================
-- 3. ENTRY POINT  sp_seed_creative_clones_ctv_poc_pol(p_mode)
...
--    Run adhoc:  CALL tempwork.sp_seed_creative_clones_ctv_poc_pol('ALL');
-- =====================================================================================
CREATE OR REPLACE PROCEDURE tempwork.sp_seed_creative_clones_ctv_poc_pol(IN p_mode character varying DEFAULT 'ALL')
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    ...
    IF v_mode IN ('ALL', 'NEW') THEN
        ...
        FROM tempwork.watermark_control_ctv_poc_pol WHERE watermark_name = 'CTV_POC_SEED_NEW';

        SELECT max(updated_timestamp) INTO v_now FROM tempwork.creative_staging_ctv_poc_pol;
        ...
        IF v_now IS NOT NULL AND v_now > v_last THEN
            ...
            FROM tempwork.creative_staging_ctv_poc_pol s
            WHERE s.updated_timestamp > v_last;

            GET DIAGNOSTICS v_rows = ROW_COUNT;
            RAISE NOTICE 'MODE 1 (new inserts): % candidate anchor(s) since %', v_rows, v_last;

            CALL tempwork.sp_seed_load_footprint_ctv_poc_pol();

            UPDATE tempwork.watermark_control_ctv_poc_pol
    ...
    IF v_mode IN ('ALL', 'UPDATE') THEN
        SELECT coalesce(table_tx_end, '1900-01-01'::timestamp) INTO v_last
        FROM tempwork.watermark_control_ctv_poc_pol WHERE watermark_name = 'CTV_POC_SEED_UPDATE';
        ...
        IF v_now IS NOT NULL AND v_now > v_last THEN
            ...
            FROM tempwork.creative_dedupe_map_ctv_poc_pol dm
