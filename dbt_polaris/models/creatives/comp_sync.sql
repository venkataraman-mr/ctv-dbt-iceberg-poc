{#
  ...
  POLARIS: catalog literal iceberg.* -> polaris.*. attribute_response / attribute_response_vx2 arrive from
  comp_sync_revxlate already as VARIANT, so `select *` writes variant -> variant (gold.component_coding v3).
#}

-- depends_on: {{ ref('comp_sync_revxlate') }}

{%- set cand = 'polaris.bronze.comp_sync_revxlate' -%}

{%- set merge_sql %}
merge into polaris.gold.component_coding target
...
{%- set hold_sql %}
merge into polaris.silver.component_coding_translation_hold t
...
{{ config(
    ...
    post_hook=[
      merge_sql,
      hold_sql,
      "{{ watermark_ts_finish_from_relation('CTV_SYNC_COMPONENT', 'polaris.bronze.comp_sync_revxlate', 'modified_timestamp') }}"
    ]
) }}
