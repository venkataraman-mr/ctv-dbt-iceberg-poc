{#
  ...
  push succeeds):
    ...
    3 drop + 4 create PG temp      -> tempwork.tmp_digital_raw_occ_to_crtv_staging_ctv_poc_pol (cross-catalog CTAS)
...
{%- set tmp_pg = 'tempwork.tmp_digital_raw_occ_to_crtv_staging_ctv_poc_pol' -%}
...
{#- Relations used inside post_hooks MUST be plain literal strings, not source()/ref() Relation
    ...
    {% set %} degrades to the model's own relation (polaris.silver.<model>) on that re-render, while a
...
{%- set rel_uu               = 'polaris.bronze.creative_unique_urls' -%}
{%- set rel_autochaff        = 'polaris.bronze.creative_autochaff' -%}
{%- set rel_autochaff_recs   = 'polaris.bronze.crtv_autochaff_records' -%}
{%- set rel_data_provider    = 'polaris.km_preparation_db.data_provider' -%}
...
{%- set h_call = pg_call("call tempwork.sp_dbx_digital_insert_crtv_staging_first_seen_ctv_poc_pol('" ~ tmp_pg ~ "')") -%}
