{#
  ...
  Post-hooks (in order), all gated on the reverse-translation holding_flag:
    1. gold MERGE  -> polaris.gold.creative (107 cols; the 12 VARIANT gold columns are written via
       CAST(json_parse(<varchar json>) AS variant); exact Databricks UPDATE-SET subset so immutable/identity
       cols are preserved on match; updated_timestamp := current_timestamp (MRVXVC-14938); first_seen_occurrence_id
       keeps the existing value when the incoming is null). Only holding_flag=false rows (source is pre-filtered).
    2. change log  -> polaris.silver.gold_creative_change_log (json_log is VARIANT: the json_object over the
       Databricks json_log_columns is built from the candidate's VARCHAR json strings, then wrapped
       cast(json_parse(cast(... as varchar)) as variant); timestamps ->varchar; absent-on-null).
    3. translation hold MERGE -> polaris.silver.creative_mapping_translation_hold: park held
    ...
    4. CE-holding merge-back (Postgres, via pg_call/system.execute): add held / remove processed in
       tempwork.creative_classification_engine_holding_ctv_poc_pol from the proc scratch
       crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol (its own proc-computed hold_flag).
  ...
  POLARIS: (1) catalog literal iceberg.* -> polaris.*; (2) Postgres tempwork clones *_ctv_poc -> *_ctv_poc_pol;
  (3) the 12 gold.creative VARIANT columns are cast varchar-json -> variant in the merge source (the candidate
  crtv_sync_creative_revxlate keeps them VARCHAR); (4) the change-log json_log (VARIANT) is wrapped to variant.
  The change_log json_object still reads the candidate's VARCHAR columns (json_parse there would yield json-typed
  values json_object can't serialize) -- only the final json_log result is cast to variant.
#}

-- depends_on: {{ ref('crtv_sync_creative_revxlate') }}

{%- set gold_merge_sql %}
merge into polaris.gold.creative t
    using (
    select
      ...
      cast(json_parse(secondary_products) as variant) as secondary_products,
      cast(json_parse(vx1_secondary_products) as variant) as vx1_secondary_products,
      cast(json_parse(vx2_secondary_products) as variant) as vx2_secondary_products,
      cast(null as variant) as mr_secondary_company_ids,
      ...
      cast(json_parse(attribution_competitor) as variant) as attribution_competitor,
      case when json_extract_scalar(attribution_celebrity, '$[0].id') is null then null else cast(json_parse(attribution_celebrity) as variant) end as attribution_celebrity,
      ...
      cast(json_parse(custom_attributes) as variant) as custom_attributes,
      ...
      cast(json_parse(attribution_competitor_vx2) as variant) as attribution_competitor_vx2,
      ...
      cast(json_parse(creative_payload) as variant) as creative_payload,
      cast(json_parse(machine_learning_payload) as variant) as machine_learning_payload,
      ...
      cast(json_parse(print_matching_ads) as variant) as print_matching_ads,
      cast(json_parse(print_ad_images) as variant) as print_ad_images,
    ...
    from polaris.bronze.crtv_sync_creative_revxlate
    where holding_flag = false
    ) s
    on t.creative_id = s.creative_id and t.creative_url_hash = s.creative_url_hash
    when matched then update set
      creative_duration = s.creative_duration,
      creative_duration_bucket = s.creative_duration_bucket,
      creative_tier_id = s.creative_tier_id,
      primary_product_id = s.primary_product_id,
      vx1_product_id = s.vx1_product_id,
      vx2_product_id = s.vx2_product_id,
      mr_company_id = s.mr_company_id,
      secondary_products = s.secondary_products,
      vx1_secondary_products = s.vx1_secondary_products,
      vx2_secondary_products = s.vx2_secondary_products,
      mr_secondary_company_ids = s.mr_secondary_company_ids,
      primary_language_code = s.primary_language_code,
      classification_type = s.classification_type,
      classified_by_user_id = s.classified_by_user_id,
      classification_comments = s.classification_comments,
      classified_timestamp = s.classified_timestamp,
      updated_timestamp = s.updated_timestamp,
      classification_process_step = s.classification_process_step,
      creative_title = s.creative_title,
      creative_headline = s.creative_headline,
      attribution_first_audio = s.attribution_first_audio,
      attribution_lead_text = s.attribution_lead_text,
      attribution_visual = s.attribution_visual,
      attribution_summary = s.attribution_summary,
      attribution_other_details = s.attribution_other_details,
      attribution_description = s.attribution_description,
      attribution_hashtag = s.attribution_hashtag,
      attribution_competitor = s.attribution_competitor,
      attribution_celebrity = s.attribution_celebrity,
      attribution_slogan_tagline = s.attribution_slogan_tagline,
      attribution_revision_description = s.attribution_revision_description,
      attribution_comments = s.attribution_comments,
      attribution_creative_tags = s.attribution_creative_tags,
      custom_attributes = s.custom_attributes,
      attribution_timestamp = s.attribution_timestamp,
      attribution_by_user_id = s.attribution_by_user_id,
      attribution_status = s.attribution_status,
      is_sponsored_video = s.is_sponsored_video,
      is_component_eligible = s.is_component_eligible,
      component_entry_status = s.component_entry_status,
      component_entry_by_user_id = s.component_entry_by_user_id,
      component_entry_timestamp = s.component_entry_timestamp,
      additional_multi_product_flag = s.additional_multi_product_flag,
      additional_coop_product_flag = s.additional_coop_product_flag,
      send_to_adscope_unattributed = s.send_to_adscope_unattributed,
      first_seen_media = s.first_seen_media,
      first_seen_provider_occurrence_id = s.first_seen_provider_occurrence_id,
      first_seen_occurrence_id = case when s.first_seen_occurrence_id is not null then s.first_seen_occurrence_id else t.first_seen_occurrence_id end,
      first_seen_occurrence_timestamp = s.first_seen_occurrence_timestamp,
      first_seen_provider_code = s.first_seen_provider_code,
      first_seen_media_property_id = s.first_seen_media_property_id,
      first_seen_media_property_name = s.first_seen_media_property_name,
      first_seen_media_category_id = s.first_seen_media_category_id,
      first_seen_media_category_code = s.first_seen_media_category_code,
      first_seen_provider_creative_link_url = s.first_seen_provider_creative_link_url,
      first_seen_provider_publisher_id = s.first_seen_provider_publisher_id,
      first_seen_provider_publisher_domain = s.first_seen_provider_publisher_domain,
      first_seen_provider_campaign_id = s.first_seen_provider_campaign_id,
      first_seen_provider_campaign_name = s.first_seen_provider_campaign_name,
      first_seen_provider_advertiser_id = s.first_seen_provider_advertiser_id,
      first_seen_provider_advertiser_name = s.first_seen_provider_advertiser_name,
      first_seen_provider_product_id = s.first_seen_provider_product_id,
      first_seen_provider_product_name = s.first_seen_provider_product_name,
      first_seen_provider_campaign_landing_page = s.first_seen_provider_campaign_landing_page,
      first_seen_market_id = s.first_seen_market_id,
      first_seen_market_name = s.first_seen_market_name,
      first_seen_daypart_id = s.first_seen_daypart_id,
      first_seen_daypart_name = s.first_seen_daypart_name,
      first_seen_affiliate_id = s.first_seen_affiliate_id,
      first_seen_affiliate_name = s.first_seen_affiliate_name,
      due_timestamp = s.due_timestamp,
      last_seen_timestamp = s.last_seen_timestamp,
      print_ad_nli = s.print_ad_nli,
      print_ad_equ = s.print_ad_equ,
      print_ad_cost = s.print_ad_cost,
      print_ad_size = s.print_ad_size,
      print_null_cost_comments = s.print_null_cost_comments,
      print_los_id = s.print_los_id,
      print_ad_type_id = s.print_ad_type_id,
      print_ad_col_inch = s.print_ad_col_inch,
      print_ad_weighted_col_inch = s.print_ad_weighted_col_inch,
      print_recalculate_cost = s.print_recalculate_cost,
      print_matching_ads = s.print_matching_ads,
      print_ad_images = s.print_ad_images,
      is_resegment = s.is_resegment,
      product_mapping_status = s.product_mapping_status,
      is_reclassified = s.is_reclassified,
      print_no_cost = s.print_no_cost,
      attribution_competitor_vx2 = s.attribution_competitor_vx2,
      creative_payload = s.creative_payload,
      machine_learning_payload = s.machine_learning_payload
...
{%- endset %}
{%- set change_log_sql %}
insert into polaris.silver.gold_creative_change_log
    ...
    select creative_id, creative_url_hash,
      ...
      cast(json_parse(cast(json_object(
        ...
        absent on null
      ) as varchar)) as variant) as json_log
    from polaris.bronze.crtv_sync_creative_revxlate
    where holding_flag = false
{%- endset %}
{%- set translation_hold_sql %}
merge into polaris.silver.creative_mapping_translation_hold t
    using (select distinct creative_id, holding_flag from polaris.bronze.crtv_sync_creative_revxlate) s
...
{%- endset %}
{%- set ce_hold_pg_sql %}merge into tempwork.creative_classification_engine_holding_ctv_poc_pol t using tempwork.crtv_sync_to_db_updated_crtvs_with_holding_flag_ctv_poc_pol s on s.creative_id = t.creative_id when matched and s.hold_flag = false then delete when not matched and s.hold_flag = true then insert (creative_id, inserted_at) values (s.creative_id, clock_timestamp() at time zone 'UTC'){%- endset %}
...
{{ config(
    ...
    post_hook=[
      ...
      "{{ watermark_ts_finish_from_relation('CTV_SYNC_CREATIVE', 'polaris.bronze.crtv_sync_creative_revxlate', 'updated_timestamp', 'holding_flag = false') }}"
    ]
) }}
