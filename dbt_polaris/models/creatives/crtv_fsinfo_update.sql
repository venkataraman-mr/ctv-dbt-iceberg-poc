{#
  ...
  POLARIS: only scalar gold columns are read/written here (no VARIANT column touched), so the sole change
  from Nessie is the catalog literal iceberg.* -> polaris.*.
#}
...
{%- set self_rel = 'polaris.bronze.' ~ this.identifier -%}   {#- literal: ref()/this degrade to silver in a hook -#}

{%- set merge_sql %}
merge into polaris.gold.creative m
using {{ self_rel }} s
...
{{ config(
    ...
    post_hook=[
      merge_sql,
      "{{ watermark_ts_advance_from_source('CTV_FSINFO_FROM_FIRSTSEEN', 'polaris.gold.creative_first_seen', 'updated_timestamp') }}",
      "{{ watermark_ts_advance_from_source('CTV_FSINFO_FROM_CREATIVE', 'polaris.gold.creative', 'updated_timestamp') }}"
    ]
) }}

with changed as (
    select creative_id
    from polaris.gold.creative_first_seen
...
resolved_parent as (
    ...
    from changed cc
    left join polaris.silver.creative_dedupe_map cdm
...
fetch_entire_family as (
    ...
    join polaris.silver.creative_dedupe_map cdm
...
family_first_seen as (
    select * from (
        ...
        inner join polaris.gold.creative_first_seen s
