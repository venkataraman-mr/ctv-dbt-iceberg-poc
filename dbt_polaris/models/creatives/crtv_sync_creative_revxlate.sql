{#
  ...
  POLARIS: identical to Nessie -- pure ref()/source(). It reads crtv_sync_creative_raw, whose json columns are
  VARCHAR json strings, so json_parse works unchanged; the vxN arrays it builds stay VARCHAR json. The VARCHAR->
  variant cast happens at the gold write in crtv_sync_creative. No variant/smallint edits here.
#}
