{#
  ...
  POLARIS: only the Postgres tempwork source clone changes (*_ctv_poc -> *_ctv_poc_pol). attribute_response is
  read from Postgres jsonb (not an Iceberg VARIANT column) -> the json_format/json_parse handling is unchanged.
#}
