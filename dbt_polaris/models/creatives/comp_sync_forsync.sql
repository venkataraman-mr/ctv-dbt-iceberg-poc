{#
  ...
  Mirrors PsqlComponentSync.read_changes_from_psql:
    pre-hook 1-2: (re)build the component-hold clone tempwork.component_hold_creative_tmp_ctv_poc_pol from the
    ...
    pre-hook 3:   CALL the cloned component proc (run-time watermark via p4_component_proc_call) ->
                  builds tempwork.component_coding_forsync_tmp_ctv_poc_pol.
  Body: 1-row summary of the forsync. Transforms + writes are stages 2-4 (comp_sync_*), which depend on this.

  Prereqs: ddl/postgres/polaris/piece4_sync_procs_ctv_poc.sql (component proc clone); CTV_SYNC_COMPONENT seeded (ddl/polaris/08).
...
{%- set comp_hold = 'postgres.tempwork.component_hold_creative_tmp_ctv_poc_pol' -%}
{%- set hold_src  = 'polaris.silver.component_coding_translation_hold' -%}
