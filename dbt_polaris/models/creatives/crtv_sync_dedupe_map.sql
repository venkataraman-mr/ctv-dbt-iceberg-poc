{#
  ...
  tempwork.creative_dedupe_map_ctv_poc_pol, resolve match_type_id -> match_type NAME via
  ...
  polaris.silver.creative_dedupe_map on (child_creative_id, child_creative_url_hash).
  ...
  Types cast to the silver target (ids BIGINT; scores REAL; json_response VARIANT; naive Postgres
  timestamps -> timestamp(6) with time zone). Delete pass (removed children) is crtv_sync_dedupe_map_delete.

  Prereq (once): seed CTV_SYNC_DEDUP_UPSERT (ddl/polaris/08).

  POLARIS: (1) catalog literal iceberg.* -> polaris.*; (2) Postgres tempwork clone *_ctv_poc -> *_ctv_poc_pol;
  (3) json_response is CAST to variant (silver.py has it VARIANT; Nessie stored VARCHAR). cdm.json_response is
  Postgres jsonb -> Trino json, and cast(json AS variant) writes it to the v3 variant column.
#}

{%- set wm_name = 'CTV_SYNC_DEDUP_UPSERT' -%}
{%- set src = source('tempwork', 'creative_dedupe_map_ctv_poc_pol') -%}
{%- set mt  = source('reference', 'creative_match_type') -%}   {#- Iceberg (UC reference sync) -#}
{%- set tgt = 'polaris.silver.creative_dedupe_map' -%}
...
select
    ...
    cast(cdm.json_response as variant)                               as json_response,   -- source VARIANT (jsonb -> Trino json -> variant)
