{#
  ...
  POLARIS: (1) catalog literal iceberg.* -> polaris.*; (2) the gold.creative vx1/vx2_secondary_products columns
  and the silver resync-log secondary_products/vx1/vx2/mr_secondary columns are VARIANT -- the VARCHAR json
  strings carried in the bronze self table are CAST to variant (cast(json_parse(...) as variant); null->variant)
  at the MERGE and the INSERT. The bronze self table itself stays VARCHAR json (no variant column on it).
#}
...
{%- set merge_sql %}
merge into polaris.gold.creative target
...
when matched then update set
  ...
  vx1_secondary_products = cast(json_parse(source.new_vx1_secondary_products) as variant),
  vx2_secondary_products = cast(json_parse(source.new_vx2_secondary_products) as variant)
{%- endset %}

{%- set log_sql %}
insert into polaris.silver.creative_product_translation_resync_log
...
select
  ...
  cast(json_parse(secondary_products) as variant),
  cast(json_parse(old_vx1_secondary_products) as variant),
  cast(json_parse(old_vx2_secondary_products) as variant),
...
{{ config(
    ...
    post_hook=[
      merge_sql,
      log_sql,
      "{{ watermark_ts_advance_from_source('CTV_PRODUCT_RESYNC', 'polaris.productcentral.productmap', 'change_dt') }}"
    ]
) }}
