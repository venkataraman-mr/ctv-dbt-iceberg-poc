{#
  ...
  POLARIS: (1) Postgres tempwork source clone *_ctv_poc -> *_ctv_poc_pol; (2) attribute_response /
  attribute_response_vx2 are emitted as VARIANT here (gold.component_coding has them VARIANT; comp_sync then
  writes them straight through) via cast(<array json> AS variant); (3) component_template_id / sequence / share /
  page_no are CAST to INTEGER (Nessie used SMALLINT) -- this intermediate Iceberg table cannot hold smallint.
#}
...
rebuilt as (
    select
        ...
        cast(cast(array_agg(vx0_elem order by seq) as json) as variant) as attribute_response,
        case when max(status) = 'Coding Completed'
             then cast(cast(array_agg(vx2_elem order by seq) as json) as variant) end as attribute_response_vx2,
...
select
    ...
    cast(f.component_template_id as integer)                    as component_template_id,   -- source int2 -> INTEGER
    cast(f.component_template_name as varchar)                  as component_template_name,
    cast(f.sequence as integer)                                 as sequence,               -- source int2 -> INTEGER
    cast(f.share as integer)                                    as share,                  -- source int2 -> INTEGER
    ...
    cast(f.page_no as integer)                                  as page_no,                -- source int2 -> INTEGER
