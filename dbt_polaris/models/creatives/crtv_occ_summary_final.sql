...
{%- set cand = 'polaris.bronze.crtv_occ_summary_candidate' -%}
{%- set missing_rel = 'polaris.bronze.missing_digital_occurrence_for_summary' -%}
{%- set self_rel = this.database ~ '.bronze.' ~ this.identifier -%}
{%- set tmp_pg = 'tempwork.tmp_raw_occ_for_crtv_occ_summary_ctv_poc_pol' -%}
...
from {{ self_rel }}
{%- endset -%}
{%- set h_call = pg_call("call tempwork.sp_dbx_digital_upsert_to_crtv_occ_summary_ctv_poc_pol('" ~ tmp_pg ~ "')") -%}
{%- set h_merge_missing -%}
