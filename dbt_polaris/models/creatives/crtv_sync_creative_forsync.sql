{#
  ...
  Mirrors PsqlCrtvSync steps 2.1–2.3:
    pre-hook 1-2: (re)build the advert-hold clone tempwork.creatives_advert_hold_tmp_ctv_poc_pol from the
    ...
    pre-hook 3:   CALL the cloned proc (run-time watermark via p4_creative_proc_call — pre_hook is captured
                  at PARSE so the watermark must be read at run) → builds tempwork.creative_forsync_tmp_ctv_poc_pol.
  ...
  Prereqs (once): ddl/postgres/polaris/piece4_sync_procs_ctv_poc.sql (proc clones); CTV_SYNC_CREATIVE seeded (ddl/polaris/08).
  VALIDATE ON VM: the advert-hold CTAS + the cross-catalog CALL succeed and forsync populates.

  POLARIS: catalog literal iceberg.* -> polaris.*; Postgres tempwork clones *_ctv_poc -> *_ctv_poc_pol.
#}
...
{%- set advert_hold = 'postgres.tempwork.creatives_advert_hold_tmp_ctv_poc_pol' -%}
{%- set hold_src = 'polaris.silver.creative_mapping_translation_hold' -%}
