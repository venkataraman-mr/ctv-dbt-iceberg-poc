{#
  ...
  tempwork.creative_first_seen_ctv_poc_pol and MERGE into polaris.gold.creative_first_seen on
  ...
  Prereqs (once, on the VM): seed CTV_SYNC_FIRST_SEEN (ddl/polaris/08) and
  ...
  (3) provider_id / market_id / daypart_id are CAST to INTEGER (Nessie cast them to SMALLINT) -- the gold
  target columns are INTEGER on Polaris and this intermediate Iceberg table cannot hold smallint.
#}

{%- set wm_name = 'CTV_SYNC_FIRST_SEEN' -%}
{%- set src = source('tempwork', 'creative_first_seen_ctv_poc_pol') -%}
{%- set tgt = 'polaris.gold.creative_first_seen' -%}
...
select
    ...
    cast(provider_id                 as integer)                      as provider_id,     -- source int2 -> INTEGER
    ...
    cast(market_id                   as integer)                      as market_id,       -- source int2 -> INTEGER
    cast(market_name                 as varchar)                      as market_name,
    cast(daypart_id                  as integer)                      as daypart_id,      -- source int2 -> INTEGER
