{#
  ...
  Watermark CTV_PRODUCT_RESYNC (change_dt). MUST be initialized to max(change_dt), not 1900 -- see ddl/polaris/08.
  ...
  POLARIS: (1) catalog literal iceberg.* -> polaris.*; (2) gold.creative.secondary_products is now a VARIANT
  column -- it is read via cast(... AS json) (NOT json_parse, which rejects variant) to build sec_pid_array,
  and the carried-forward secondary_products / old_vx1/vx2_secondary_products are DOWN-CAST to VARCHAR json
  strings (json_format(cast(... as json))) so the downstream _prim/_sec/_resync logic stays identical to Nessie.
#}
...
{%- set hold = 'polaris.silver.creative_mapping_translation_hold' -%}
...
select
    ...
    c.mr_company_id,
    -- down-cast the gold VARIANT columns back to VARCHAR json for the downstream (Nessie-identical) logic
    json_format(cast(c.secondary_products as json))     as secondary_products,
    c.vx1_product_id            as old_vx1_product_id,
    c.vx2_product_id            as old_vx2_product_id,
    json_format(cast(c.vx1_secondary_products as json)) as old_vx1_secondary_products,
    json_format(cast(c.vx2_secondary_products as json)) as old_vx2_secondary_products,
