{#
  ...
  POLARIS: only scalar gold columns touched -> sole change from Nessie is iceberg.* -> polaris.*.
#}
...
{{ config(
    ...
    post_hook=[
      merge_sql,
      "{{ watermark_ts_advance_from_source('CTV_LAST_SEEN_DIGITAL', 'polaris.gold.digital_gold_occurrence', 'updated_timestamp') }}"
    ]
) }}

with changed as (
    select distinct creative_id as parent_creative_id
    from polaris.gold.digital_gold_occurrence
...
all_occ as (
    ...
    from changed c
    inner join polaris.gold.digital_gold_occurrence o
