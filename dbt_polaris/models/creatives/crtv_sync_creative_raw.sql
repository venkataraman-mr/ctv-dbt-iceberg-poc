{#
  ...
  re-parses secondary_products for reverse translation; stage 3 CASTs them to variant at the gold write. Depends on
  ...
  POLARIS: (1) Postgres tempwork source clone *_ctv_poc -> *_ctv_poc_pol; (2) the 3 Postgres int2 columns
  (creative_tier_id / first_seen_market_id / first_seen_daypart_id) are CAST to INTEGER here -- arbitrary()
  would otherwise leave them SMALLINT, which the Polaris/Iceberg REST connector rejects when this model
  materializes as an Iceberg table ("Type not supported for Iceberg: smallint"). The json columns stay VARCHAR.
#}
-- depends_on: {{ ref('crtv_sync_creative_forsync') }}

{%- set fs = source('tempwork', 'creative_forsync_tmp_ctv_poc_pol') -%}
...
select
    ...
    cast(arbitrary(c.creative_tier_id) as integer) as creative_tier_id,   -- source int2 -> INTEGER (Iceberg has no smallint)
    ...
    cast(arbitrary(c.first_seen_market_id) as integer) as first_seen_market_id,     -- source int2 -> INTEGER
    arbitrary(c.first_seen_market_name) as first_seen_market_name,
    cast(arbitrary(c.first_seen_daypart_id) as integer) as first_seen_daypart_id,   -- source int2 -> INTEGER
