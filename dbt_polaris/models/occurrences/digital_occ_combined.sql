{#
  ...
  POLARIS variant handling: daisy_chain and raw_json are VARIANT (from digital_occ_raw_cdf) and provider_raw_json
  is VARIANT on silver.digital_staging_occurrence.
    * daisy_chain stays variant through the UNION (both sides variant); read it with cast(... AS json) for the
      in-place transform (NOT json_parse).
    * raw_json (BISSocial branch) read with cast(raw_json AS json) (NOT json_parse -- rejects variant).
    * provider_raw_json is kept as a VARCHAR json string in THIS model (raw side = json_object -> varchar;
      staging side down-cast json_format(cast(variant AS json))), so the UNION types match and the occ_daisy
      json_extract_scalar reads work unchanged. It is CAST to variant only at the gold/staging writes (digital_occ_gold).
    * format_version=3: the body carries the variant daisy_chain, so the CTAS must create a v3 table.
  Catalog literals iceberg.* -> polaris.*.
#}

-- depends_on: {{ ref('digital_occ_raw_cdf') }}

{%- set raw = ref('digital_occ_raw_cdf') -%}
{%- set staging = 'polaris.silver.digital_staging_occurrence' -%}
{%- set dedupe  = 'polaris.silver.creative_dedupe_map' -%}
...
{{ config(materialized='table', schema='bronze', tags=['occurrences', 'DIGITAL_RAW_OCC_TO_GOLD_OCC'], views_enabled=false, on_table_exists='drop', properties={'format_version': '3'}) }}
    ...
    select
        ...
        case when provider_code = {{ source_bis_social_code }} then
            json_object(
                ...
                'MAIN_ADVERTISER_CATEGORY'   value coalesce(json_extract_scalar(try(cast(raw_json as json)), '$.mainAdvertiserCategory'), 'NA'),
                'SECOND_ADVERTISER_CATEGORY' value coalesce(json_extract_scalar(try(cast(raw_json as json)), '$.secondAdvertiserCategory'), 'NA'),
                'OCCURRENCE_SOCIAL_PAGE_LINK'      value coalesce(json_extract_scalar(try(cast(raw_json as json)), '$.occurrence.socialPageLink'), 'NA'),
                'OCCURRENCE_SOCIAL_PAGE_NAME'      value coalesce(json_extract_scalar(try(cast(raw_json as json)), '$.occurrence.socialPageName'), 'NA'),
                'OCCURRENCE_SOCIAL_CAMPAIGN_TEXT'  value coalesce(json_extract_scalar(try(cast(raw_json as json)), '$.occurrence.socialCampaignText'), 'NA'),
                'OCCURRENCE_CREATIVEURL' value coalesce(json_extract_scalar(try(cast(raw_json as json)), '$.occurrence.creativeUrl'), 'NA'),
                'FIRST_SEEN_DATE'   value coalesce(json_extract_scalar(try(cast(raw_json as json)), '$.occurrence.creative.firstSeenDate'), 'NA')
            )
        else
            json_object(
...
staging_side as (
    select
        ...
        false as delete_flag,
        ad_insertion_point,
        json_format(cast(provider_raw_json as json)) as provider_raw_json,   -- variant -> VARCHAR json (match raw_side for the UNION)
        daisy_chain,
...
occ_daisy as (
    select
        ...
        array_join(transform(array_sort(transform(
            cast(cast(d.daisy_chain as json) as array(json)),
