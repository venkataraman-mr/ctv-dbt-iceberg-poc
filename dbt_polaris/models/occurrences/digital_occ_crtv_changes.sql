{#
  ...
  POLARIS: only scalar gold.creative columns are read (no variant), so the sole change vs Nessie is the catalog
  literal iceberg.* -> polaris.*. Scalar body -> no format_version needed.
#}
...
{%- set gold_occ = 'polaris.gold.digital_gold_occurrence' -%}
...
{{ config(
    ...
    post_hook=[
      reparent_merge,
      delete_flag_merge,
      "{{ watermark_ts_advance_from_source('DIGITAL_CRTV_CHANGES_TO_GOLD_OCC', 'polaris.gold.creative', 'updated_timestamp') }}"
    ]
) }}
