{#
  ...
  POLARIS: (1) catalog literal iceberg.* -> polaris.*; (2) media_property_flatten_vx0_vw is a real Iceberg VIEW on
  Polaris (Step 1) -> source() not ref(); (3) only SCALAR gold.creative columns are read (no variant read here);
  daisy_chain (variant) and provider_raw_json (VARCHAR json here) pass through untouched -> format_version=3 because
  the body table carries the variant daisy_chain.
#}

-- depends_on: {{ ref('digital_occ_combined') }}
-- depends_on: {{ ref('digital_occ_deploychain') }}

{%- set creative = 'polaris.gold.creative' -%}
{%- set deploy   = 'polaris.gold.digital_deployment_chain' -%}
{%- set origin   = source('km_preparation_db', 'origin') -%}
{%- set pflat    = source('productcentral', 'product_flatten') -%}
{%- set mpf_vw   = source('km_preparation_gold_db', 'media_property_flatten_vx0_vw') -%}
