{#
  ...
  daisy_chain = array<struct<id, role, index, roleId, mediator>>. Per DISTINCT chain, sorted by
  ...
  POLARIS: (1) catalog literal iceberg.* -> polaris.*; (2) daisy_chain is VARIANT (from digital_occ_raw_cdf) --
  read it with cast(daisy_chain AS json) NOT json_parse (json_parse rejects variant); (3) the gold merge writes
  purchase_method_id as INTEGER (Nessie cast to smallint; gold.digital_deployment_chain is INTEGER on Polaris);
  (4) format_version=3 -- this model's body carries the variant daisy_chain, so the CTAS must create a v3 table.
#}

-- depends_on: {{ ref('digital_occ_raw_cdf') }}

{%- set raw = 'polaris.bronze.digital_occ_raw_cdf' -%}
{%- set self_rel = 'polaris.bronze.' ~ this.identifier -%}

{%- set chain_merge %}
merge into polaris.gold.digital_deployment_chain tgt
using (
...
values (
    ...
    cast(src.purchase_method_id as integer), src.md5_hashcode, cast(current_timestamp as timestamp(6) with time zone))
{%- endset %}

{%- set role_merge %}
merge into polaris.gold.digital_deployment_chain_role tgt
using (
    select distinct
    ...
    cross join unnest(cast(cast(dc.daisy_chain as json) as array(json))) as u(elem)
) src
...
{%- set mediator_merge %}
merge into polaris.gold.digital_deployment_chain_mediator tgt
using (
    select distinct
...
parsed as (
    select
        ...
        array_sort(transform(
            cast(cast(daisy_chain as json) as array(json)),
            e -> cast(row(
