{#
  ...
  tempwork.occurrence_id_seq_ctv_poc_pol (START 75,000,000,000) via reserve_occurrence_ids(n); assigned
  ...
  PREREQ: ddl/postgres/polaris/piece5_occ_id_seq_ctv_poc.sql (the 75B *_ctv_poc_pol sequence + reservation proc).

  POLARIS: (1) catalog literal iceberg.* -> polaris.*; (2) provider_raw_json is VARIANT on both gold.digital_gold_occurrence
  and silver.digital_staging_occurrence -- it is carried as a VARCHAR json string through this model and CAST to
  variant (cast(json_parse(...) as variant)) at BOTH merge writes; daisy_chain (variant) writes straight through to
  staging. (3) market_id / purchase_method_id / origin_channel_id are cast to INTEGER (Nessie used SMALLINT).
  This model's body table has no variant column (provider_raw_json is VARCHAR here) -> no format_version override needed.
#}

-- depends_on: {{ ref('digital_occ_classified') }}

{%- set cls = 'polaris.bronze.digital_occ_classified' -%}
{%- set self_rel = 'polaris.bronze.' ~ this.identifier -%}
...
{%- set gold_merge %}
merge into polaris.gold.digital_gold_occurrence goc
using {{ self_rel }} spv
...
values (
    ...
    spv.provider_campaign_landing_page_domain, cast(json_parse(spv.provider_raw_json) as variant), spv.ad_insertion_point, spv.job_log_key)
{%- endset %}

{%- set staging_merge %}
merge into polaris.silver.digital_staging_occurrence target
using {{ cls }} source
...
values (
    ...
    source.provider_campaign_landing_page_domain, source.ad_insertion_point, source.job_log_key, cast(json_parse(source.provider_raw_json) as variant))
{%- endset %}
...
select
    ...
    cast(market_id as integer)                               as market_id,            -- source SMALLINT -> INTEGER
    cast(purchase_method_id as integer)                      as purchase_method_id,   -- source SMALLINT -> INTEGER
    cast(deployment_chain_id as bigint)                      as deployment_chain_id,
    mediator_chain,
    cast(origin_channel_id as integer)                       as origin_channel_id,    -- source SMALLINT -> INTEGER
    prelim_impressions,
    prelim_spend,
