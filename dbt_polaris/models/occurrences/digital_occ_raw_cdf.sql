{#
  ...
  POLARIS: daisy_chain + raw_json are VARIANT on bronze.digital_raw_occurrence and pass through this model
  UNCHANGED (plain column select -- no json functions here), so the ONLY change vs Nessie is
  format_version=3 on this model's config: its body table carries variant columns, so the dbt CTAS must
  create a v3 table (a v2 create with a variant column is rejected -- "Failed to create transaction").
#}
