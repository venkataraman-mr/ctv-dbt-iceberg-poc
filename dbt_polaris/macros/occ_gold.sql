{#
  ...
  tempwork.occurrence_id_seq_ctv_poc_pol (START 75,000,000,000) and return the block START. Exact analog of
  reserve_creative_ids (crtv_push.sql): (1) RESERVE via the writable postgres.system.execute ->
  sp_reserve_occurrence_ids_ctv_poc_pol(n) pops n values + records the block in tempwork.occurrence_id_block_ctv_poc_pol;
...
{% macro reserve_occurrence_ids(n) %}
  ...
  {% set do_reserve %}
    call postgres.system.execute(query => 'call tempwork.sp_reserve_occurrence_ids_ctv_poc_pol({{ cnt }})')
  {% endset %}
  {% do run_query(do_reserve) %}
  {% set read_q %}
    select block_start
    from table(postgres.system.query(query =>
      'select block_start from tempwork.occurrence_id_block_ctv_poc_pol order by block_id desc limit 1'))
  {% endset %}
