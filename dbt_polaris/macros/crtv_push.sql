{#
  ...
  (DIGITAL_RAW_OCC_TO_CRTV_FIRST_SEEN_UPDATE):
    ...
    * reserve_creative_ids(n)     -> reserves a contiguous block of n ids from the Postgres sequence
                                     tempwork.creative_id_seq_ctv_poc_pol and returns the block START.
...
{#- Reserve n contiguous creative ids; return the block START (min) — the analog of the Universal
    ...
    (writable) -> the Postgres procedure sp_reserve_creative_ids_ctv_poc_pol(n) atomically pops n values
    off the sequence and records the block into tempwork.creative_id_block_ctv_poc_pol; (2) READ the newest
...
{% macro reserve_creative_ids(n) %}
  ...
  {% set do_reserve %}
    call postgres.system.execute(query => 'call tempwork.sp_reserve_creative_ids_ctv_poc_pol({{ cnt }})')
  {% endset %}
  {% do run_query(do_reserve) %}
  {% set read_q %}
    select block_start
    from table(postgres.system.query(query =>
      'select block_start from tempwork.creative_id_block_ctv_poc_pol order by block_id desc limit 1'))
  {% endset %}
